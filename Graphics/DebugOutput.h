    ...
    int sync() {
        ...
        if (this->pbase() != this->pptr()) {
            ...
            for (size_t pos = 0; pos < str.length(); pos++) {
                if (str[pos] == '\n') {
                    str[pos] = 0;
                    rc = __android_log_write(logPriority, "oblivion", str.c_str()) > 0;
                    if (str.length() > pos + 1)
...
public:
    ...
    DebugOutput()
        ...
        if (oldout != &androidCout) {
            __android_log_write(ANDROID_LOG_DEBUG, "oblivion", "redirected cout");
        }
        std::cout << "Testing cout redirect." << std::endl;
        if (olderr != &androidCerr) {
            __android_log_write(ANDROID_LOG_WARN, "oblivion", "redirected cerr");
        }
        std::cerr << "Testing cerr redirect." << std::endl;
    }
};
...
#define XR_TUT_LOG_TAG "oblivion"
#define XR_TUT_LOG(...) {                                                           \
